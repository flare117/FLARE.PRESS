...
function apply(list){ids.forEach((n,i)=>{const p=list?.[i]||{};const time=document.getElementById(`sch${n}Time`);const title=document.getElementById(`sch${n}Title`);const text=document.getElementById(`sch${n}Text`);if(time)time.textContent=p.time||'';if(title)title.textContent=p.title||'';if(text)text.textContent=p.description||p.text||p.category||'';[time,title,text].forEach(e=>e?.removeAttribute('value'))})}
...
function init(){load();setInterval(()=>{if(cached.length)apply(cached)},250);setInterval(load,60000)}
