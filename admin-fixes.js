(function(){
  ...
  if(save)save.addEventListener('click',async e=>{e.preventDefault();e.stopImmediatePropagation();const s=document.getElementById('saved');if(s)s.textContent='⏳ Сохраняем на сервер…';try{await saveServerConfig();if(typeof saveSchedule==='function')await saveSchedule();if(s)s.textContent='✓ Все изменения сохранены на сервере'}catch(x){if(s)s.textContent='❌ '+x.message}setTimeout(()=>{if(s)s.textContent=''},4000)},true);
