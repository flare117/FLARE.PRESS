...
(function(){
  ...
  function render(){
    ...
    if(diff<=0){banner.classList.add('done');banner.innerHTML='✦ <strong>NEXTV · НОВОГОДНЕЕ ОБНОВЛЕНИЕ</strong> уже запущено';return}
    const d=Math.floor(diff/86400000),h=Math.floor(diff%86400000/3600000),m=Math.floor(diff%3600000/60000);
    banner.innerHTML='❄ <strong>ГЛОБАЛЬНОЕ ОБНОВЛЕНИЕ</strong> · 1 декабря · до запуска: '+d+'д '+h+'ч '+m+'м';
  }
  document.documentElement.classList.add('nextv-winter-2026');
