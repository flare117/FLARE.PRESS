...
export default async function middleware(request){const url=new URL(request.url);const p=url.pathname;if(p==='/404.html'||p.startsWith('/api/')||p.includes('.')&&!p.endsWith('.html'))return fetch(request);if(await isMaintenance()){const u=new URL('/404.html',request.url);const page=await fetch(u);const body=await page.text();return new Response(body,{status:404,headers:{'content-type':'text/html; charset=utf-8','cache-control':'no-store'}})}return fetch(request)}
